...
interface SentTransactionSuccess {
  ...
  signature: string;
  index: number;
  timestamp: number; // Keeping for backward compatibility
  sendTimestamp: number; // New explicit send time field
}
...
interface TransactionResult {
  ...
  timestamp: number; // Keeping for backward compatibility
  index: number;
  sendTimestamp?: number; // Time when transaction was sent
  confirmTimestamp?: number; // Time of first confirmation/finalization
}

interface BenchmarkResult {
  endpoint: string;
  transactions: TransactionResult[];
  meanInclusionTime?: number; // Average time (ms) from send to confirmation
}
...
async function getServerTx(
  ...
  try {
    const response = await axios.get(
      "https://app.solayer.org/api/partner/restake/ssol",
      {
        params,
        headers: {
...
async function waitForConfirmation(
  ...
  while (attempts < maxAttempts) {
    try {
      ...
      if (
        ...
        return { success: true, confirmTimestamp: Date.now() };
      }
  ...
  return {
    success: false,
    error: "Transaction not confirmed after multiple attempts",
  };
}
...
async function runBenchmark(
  ...
  const requiredBalance = convertFromDecimalBN(amount, decimals).mul(
    new BN(numTransactions * 2)
  );
  ...
  for (let useAcceleration of [true, false]) {
    ...
    const preparedTransactions = await Promise.all(
      Array(numTransactions)
        .fill(0)
        .map(async (_, i): Promise<PreparedTransaction> => {
          try {
            ...
            console.log(
              `Transaction ${i + 1}: Amount = ${randomAmount} SOL (${(
                randomFactor * 100
              ).toFixed(2)}% of ${baseAmount} SOL)`
            );
    ...
    const sentTransactions = await Promise.all(
      preparedTransactions.map(
        async ({ transaction, index, error }): Promise<SentTransaction> => {
          ...
          try {
            ...
            console.log(
              `${index + 1}/${numTransactions}: Transaction ${signature} sent`
            );
            const now = Date.now();
            return {
              type: "success",
            ...
            console.error(
              `Error sending transaction ${index + 1}:`,
              error.message
            );
            const now = Date.now();
            return {
      ...
      sentTransactions.map(async (tx): Promise<TransactionResult> => {
        ...
        try {
          ...
          if (confirmResult.success) {
            ...
            console.log(
              `❌ Transaction ${tx.signature} failed: ${confirmResult.error}`
            );
          }
          ...
          return {
            signature: tx.signature,
            timestamp: tx.timestamp,
            sendTimestamp: tx.sendTimestamp,
            confirmTimestamp: confirmResult.confirmTimestamp,
            index: tx.index,
          ...
          console.error(
            `Error confirming transaction ${tx.signature}:`,
            error.message
          );
          return {
            signature: tx.signature,
    ...
    results[resultIndex].transactions = confirmedTransactions.sort(
      (a, b) => a.index - b.index
    );

    // Calculate mean time to inclusion for successful transactions
    const successfulTxs = results[resultIndex].transactions.filter(
      (tx) => tx.status === "success" && tx.sendTimestamp && tx.confirmTimestamp
    );

    if (successfulTxs.length > 0) {
      const totalTime = successfulTxs.reduce(
        (acc, tx) => acc + (tx.confirmTimestamp! - tx.sendTimestamp!),
        0
      );
      results[resultIndex].meanInclusionTime = totalTime / successfulTxs.length;
      console.log(
        `Mean time to inclusion (${results[resultIndex].endpoint}): ${results[
          resultIndex
        ].meanInclusionTime.toFixed(2)} ms`
      );
    } else {
      console.log(
        `No successful transactions for ${results[resultIndex].endpoint}`
      );
    }
  }
...
export async function benchmark(
  ...
  try {
    ...
    results.forEach((result) => {
      ...
      const successCount = result.transactions.filter(
    ...
    const acceleratedSuccessRate = (
      (results[0].transactions.filter((tx) => tx.status === "success").length /
        numTransactions) *
      100
    ).toFixed(2);
    const standardSuccessRate = (
      (results[1].transactions.filter((tx) => tx.status === "success").length /
        numTransactions) *
      100
    ).toFixed(2);
